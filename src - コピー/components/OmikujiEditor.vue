...
<template>
  <v-list>
    <v-list-item v-if="omikuji">
      ...
      <v-row>
        <v-col cols="12">
          <v-select v-model="omikuji.threshold.type" :items="thresholdTypes" label="種類"></v-select>
        </v-col>
      </v-row>
      ...
      <v-expansion-panels>
        <v-expansion-panel v-for="(type, index) in messageTypes" :key="index">
          ...
          <v-expansion-panel-text>
            ...
            <v-list>
              <v-list-item v-for="(post, postIndex) in omikuji[type]" :key="postIndex">
                <v-row>
...
onMounted(() => {
  console.log('OmikujiEditor: onMounted');
  updateLocalOmikuji();
});

watch(() => props.selectedItem, () => {
  console.log('OmikujiEditor: selectedItem changed', props.selectedItem);
  updateLocalOmikuji();
}, { deep: true });

function updateLocalOmikuji() {
  if (props.selectedItem?.type === 'omikuji' && props.selectedItem.index >= 0) {
    omikuji.value = { ...props.state.botMessage.omikuji[props.selectedItem.index] };
  } else {
    omikuji.value = null;
  }
  console.log('OmikujiEditor: omikuji updated', omikuji.value);
}

watch(omikuji, (newValue) => {
  console.log('OmikujiEditor: omikuji changed', newValue);
  if (newValue) {
...
const addPost = (type: MessageType) => {
  if (!Array.isArray(props.state.botMessage.omikuji[type])) {
    props.state.botMessage.omikuji[type] = [];
  }
  (props.state.botMessage.omikuji[type] as Post[]).push({
    botKey: 0,
    iconKey: '',
    delaySeconds: 0,
    content: ''
