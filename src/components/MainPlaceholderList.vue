            ...
            <v-toolbar density="compact" color="transparent">
              ...
              <template v-slot:append>
                <v-btn
                  ...
                  width="30"
                  icon
                  @click.stop="$emit('open-editor', 'placeholder', index)"
                >
...
import { ItemContent } from '@/AppTypes';
...
const emit = defineEmits<{
  (e: 'open-editor', type: 'placeholder', index: number): void;
  (e: 'update-items', items: Placeholder[]): void;
}>();

const localItems = ref<(ItemContent | { title: string; items: ItemContent[] })[]>([]);
  ...
  if (props.groupBy === 'none') {
    ...
    const grouped = props.items.reduce((acc, item) => {
      if ('name' in item && 'group' in item) {
        const key = props.groupBy === 'name' ? item.name : `グループ ${item.group}`;
        if (!acc[key]) {
          acc[key] = [];
        }
        acc[key].push(item);
    ...
    localItems.value = Object.entries(grouped).map(([title, items]) => ({ title, items }));
  }
}
