...
<template>
  ...
  <v-row dense>
    <v-col
      v-for="omikuji in omikujisNameSort"
      :key="omikuji.id"
      cols="12"
      sm="6"
      md="4"
    ...
    >
      <v-card variant="tonal">
        ...
        <v-toolbar
          density="compact"
          :color="getPostTypeColor(omikuji.post, true)"
        >
          <v-toolbar-title @click="openEditorItem('omikujis', omikuji.id)">
            <span v-if="omikuji?.threshold.length !== 0"
              >🔐</span
            >
            {{ omikuji.name }}
          </v-toolbar-title>
        ...
        <v-card-text class="py-4">
          ...
          <v-sheet class="list-group d-flex flex-wrap">
            <!-- 発動条件の表示 -->
            <v-chip
              v-if="omikuji?.threshold.length !== 0"
              density="compact"
              variant="outlined"
...
const omikujisNameSort = computed(() => {
  return Object.values(props.Omiken.omikujis).sort((a, b) => {
