...
<template>
  <v-card height="150" @click="openEditor">
    ...
    <v-card-text>
      ...
      <div class="mt-2">
        <strong>閾値タイプ:</strong> {{ getThresholdTypeLabel(item.threshold.type) }}
      </div>
      <div class="mt-2">
        <strong>閾値:</strong> {{ item.threshold.value }}
        <span v-if="item.threshold.valueMax"> - {{ item.threshold.valueMax }}</span>
      </div>
      <div class="mt-2">
        <strong>比較方法:</strong> {{ getComparisonLabel(item.threshold.comparison) }}
      </div>
    </v-card-text>
  </v-card>
</template>
...
import { EditorItem, ItemCategory, omikujiType, SelectItem, STATEType, thresholdType } from '@/types';
