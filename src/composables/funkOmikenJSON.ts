...
export function validateOmikuji(items: any | any[]): OmikujiMessage[] {
 ...
 return itemsArray.map((item: any): OmikujiMessage => ({
  ...
  message: Array.isArray(item.message) ? validatePosts(item.message) : undefined,
  party: Array.isArray(item.party) ? validatePosts(item.party) : undefined,
  toast: Array.isArray(item.toast) ? validatePosts(item.toast) : undefined,
  speech: Array.isArray(item.speech) ? validatePosts(item.speech) : undefined,
 }));
}
function validatePosts(posts: any[]): Post[] {
 return posts.map(post => ({
  // キャラクターのキー名
  botKey: typeof post.botKey === 'number' ? post.botKey : "mamono",
  // キャラクターのアイコン名
